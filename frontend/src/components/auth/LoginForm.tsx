...
const LoginForm: React.FC = () => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      // Check for 2FA requirement (case insensitive)
      if (message.toLowerCase().includes("two-factor") && message.toLowerCase().includes("required")) {
        ...
        // Keep showing 2FA field for other 2FA related errors (like invalid code)
        setRequires2FA(true);
        setError(message);
      } else {
        setError(message);
  ...
  return (
    <div className="space-y-6">
      {error && (
        <Alert variant={requires2FA ? "info" : "error"} onClose={() => setError(null)}>
          {error}
        </Alert>
      )}
